...
def render_interpretability_section(
    ...
    with col_a:
        st.image(overlay_final, caption=f"{method_final} da classe prevista ({final_class})", use_container_width=True)
    with col_b:
        st.image(boxed_final, caption="Região aproximada mais influente para a decisão", use_container_width=True)

    col_c, col_d = st.columns(2)
    with col_c:
        st.image(overlay_mel, caption="Mapa de atenção específico para melanoma", use_container_width=True)
    with col_d:
        st.image(boxed_mel, caption="Região aproximada associada à probabilidade de melanoma", use_container_width=True)
...
if mode == "Recorte manual":
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.image(draw_box_on_image(original_image, manual_box, "Recorte manual"), caption="Região selecionada", use_container_width=True)
    with c2:
        st.image(selected_image, caption="Imagem enviada ao modelo", use_container_width=True)
    ...
    if auto_box is None:
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.image(draw_box_on_image(original_image, auto_box, "Recorte automático"), caption="Bounding box aproximada por cor", use_container_width=True)
        with c2:
            st.image(selected_image, caption="Imagem enviada ao modelo", use_container_width=True)
        ...
        with st.expander("Ver máscara automática aproximada", expanded=False):
            if auto_mask is not None:
                st.image(auto_mask, caption="Máscara LAB + Otsu", use_container_width=True)
else:
    selected_image = original_image
    selected_mode = "Imagem inteira"
    st.image(original_image, caption="Imagem inteira enviada ao modelo", use_container_width=True)
...
with col_image:
    st.image(selected_image, caption=f"Imagem usada pelo modelo: {selected_mode}", use_container_width=True)
...
if compare_modes and results_to_compare:
    ...
    st.dataframe(comparison_table(results_to_compare), use_container_width=True, hide_index=True)
